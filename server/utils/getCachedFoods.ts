...
export async function getCachedFoods(
  ...
  // 嘗試用 快取的key (foodsCache)，去讀取已快取的資料，並存入 cached 變數
  const cached = await storage.getItem<ICacheData>(CACHE_KEY);

  // 若快取存在且尚未過期，直接回傳資料
  // 補充：現在時間 - 快取時間 < 24 小時
  if (cached && now - cached.timestamp < CACHE_DURATION) {
    console.log("[Cache] foodsCache 命中，直接回傳");
    return cached.data;
  }

  // 反之，若無快取或已過期，則重新向 TDX 取得 Token，再透過Token取得全台餐飲資料
  ...
  try {
    // 向 TDX 取得餐飲資料（回傳 JSON 陣列）
    const res = await $fetch<IFoodItem[]>(
      "https://tdx.transportdata.tw/api/basic/v2/Tourism/Restaurant?$format=JSON&",
      {
        headers: {
    ...
    console.error("取得餐飲資料失敗：", err);
    throw new Error("無法取得餐飲資料");
  }
}
