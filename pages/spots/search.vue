...
<template>
  <section class="mt-[40px] md:mt-[70px]">
    <!-- 麵包屑 -->
    <CommonBreadcrumb
      :items="[{ label: t('nav.home'), to: '/' }, { label: t('nav.spots') }]"
    />
    ...
    >
      ...
      <CommonInput
        :placeholder="t('search.spotPlaceholder')"
        v-model="keyword"
      />
    ...
    <div v-else>
      ...
      >
        <li v-for="item in searchResults" :key="item.id" class="w-full">
